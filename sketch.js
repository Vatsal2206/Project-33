...
function draw() {
  ...
  if(gameState === "play"){
    fill("red")
    textSize(15)
    text("**NOTE** => If your ball gets stuck somewhere press SPACE again",10,350)

    textSize(40)
    strokeWeight(3)
    stroke("cyan")
    text("Press SPACE to play!!",50,320)
  ...
  if(particle != null){
    ...
    if(particle.body.position.y > 450 && particle.body.position.y < 456){
      chance = chance - 1;
...
function keyPressed(){
  if(keyCode === 32 && gameState !== "end"){
    ...
    particle = new Ball(random(10,470),10);
  }
}